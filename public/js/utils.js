...
export function personalise(text, name) {
  ...
  return out
    .replace(/\s{2,}/g, " ")
    .replace(/\s+([,.!?:;])/g, "$1")
    // Обращение в начале фразы («Эмиль, у меня вопрос») ловится с запятой впереди, и при пустом
    // имени от него оставалась голая запятая: «, у меня вопрос». Имя по умолчанию как раз пустое
    // (store.js), так что это видел каждый, кто не вписал своё, — в том числе в озвучке.
    .replace(/^\s*,\s*/, "")
    .trim();
}
