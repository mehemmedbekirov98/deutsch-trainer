 ...
 */
// Запятая перед именем — часть обращения, и без имени она не нужна: «Привет,!» не бывает.
// Поэтому она захватывается отдельной необязательной группой ПЕРЕД проверкой границы: сама
// проверка стоит вплотную к имени, иначе «Привет, Эмиль» не совпало бы вовсе — слева от запятой
// стоит буква.
const NAME_TOKENS = /(\s*,\s*)?(?<![\p{L}\p{N}])(?:Эмиль|Emil)(?![\p{L}\p{N}@])/gu;
...
export function personalise(text, name) {
  ...
  return s
    // `comma` — то, чем обращение было отделено. С именем оно сохраняется («, Эмиль» → «, Руслан»),
    // без имени уходит вместе с ним.
    .replace(NAME_TOKENS, (m, comma) => (real ? (comma || "") + clean : ""))
    .replace(/\s{2,}/g, " ")
    .replace(/\s+([,.!?:;])/g, "$1")
    .trim();
}
