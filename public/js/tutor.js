...
export class Tutor {
  ...
  async callAi(userText) {
    ...
    try {
      ...
      // Сервер сдаётся на тридцатой секунде (см. lib/tutor.js) и успевает объяснить, почему.
      // Ждём немногим дольше — иначе браузер обрывает разговор раньше, чем приходит внятный
      // ответ об ошибке, и человек видит «нет связи» вместо «Мия задумалась».
