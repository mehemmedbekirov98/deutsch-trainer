...
export async function deleteAccount(id) {
  ...
  // Сначала сама учётная запись, потом всё остальное.
  //
  // Порядок был обратный, и это оставляло щель: строка profiles (а в ней `blocked`) исчезала
  // РАНЬШЕ учётной записи. Сорвись следующий запрос — и человек остаётся с живым токеном, но
  // без профиля, то есть разблокированным: learnerFromRequest не находит строку и честно
  // возвращает blocked: false. Удалённая учётная запись такой щели не даёт — токен перестаёт
  // проходить сразу, а остатки строк подчистятся каскадом (`on delete cascade`) и этими тремя
  // запросами следом.
  ...
  // Каскад по внешнему ключу убирает это сам — но полагаться на одно только каскадирование не
  // стоит: появится завтра таблица без такой связи, и данные тихо останутся без хозяина.
