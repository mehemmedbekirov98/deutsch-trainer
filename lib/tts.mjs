...
export async function synthesise({ text, voice, locale = "", rate = 0, pitch = 0, volume = 0, budgetMs = 0 }) {
  ...
  // Текст вставляется в SSML как есть — библиотека собирает разметку конкатенацией строк
  // (см. _SSMLTemplate в msedge-tts). Значит, «&» в немецкой фразе ломает весь запрос, а «<»
  // может открыть тег внутри нашей разметки. Экранируем здесь, а не у вызывающего: ключ кэша
  // считается от ИСХОДНОГО текста, и он должен остаться прежним.
  const ssmlText = String(text).replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c]));
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      const out = await withTimeout(readStream(tts.toStream(ssmlText, options)), streamMs, "tts timeout");
...
export async function putClip(supabaseUrl, serviceKey, key, buffer, { attempts = 4 } = {}) {
  ...
  for (let i = 0; i < attempts; i++) {
    // Everything here is worth retrying for the same reason: the audio is already synthesised,
    // so giving up throws away the expensive half of the work and leaves a line silent for
    // everybody, forever — the next run skips whatever is in the bucket and never learns that
    // this one is missing. Storage answers 429 "too_many_connections" under load, and a dropped
    // connection reaches us as a thrown fetch rather than a response.
    let r = null;
    try {
      r = await fetch(`${supabaseUrl}/storage/v1/object/tts/${storagePath(key)}`, {
        method: "POST",
        headers: {
          apikey: serviceKey,
      ...
      last = `storage: ${e?.message || e}`;
    }
    if (r?.ok || r?.status === 409) return;
    if (r) last = `storage ${r.status}: ${await r.text().catch(() => "")}`;

    const retriable = !r || r.status === 429 || r.status >= 500;
...
/**
 * Всё, что уже лежит в корзине, одним списком.
 *
 * hasClip() стоит «ничего» поштучно и очень дорого оптом: предгенерация спрашивает про 37 408
 * клипов, и по 600 мс на запрос это больше пяти часов — при том что почти все ответы «да, есть».
 * Список отдаёт по тысяче имён за запрос, то есть весь ответ примерно за минуту.
 *
 * Нужен служебный ключ: публичный листинг корзины закрыт намеренно (чтобы снаружи нельзя было
 * прочитать, какие фразы есть в курсе), и открывать его ради удобства скрипта не стоит.
 *
 * Корзина плоская, имя файла — `<ключ>.mp3`, так что ключ получается отрезанием расширения.
 */
export async function listClips(supabaseUrl, serviceKey, onPage) {
  const PAGE = 1000;
  const keys = new Set();
  for (let offset = 0; ; offset += PAGE) {
    const r = await fetch(`${supabaseUrl}/storage/v1/object/list/tts`, {
      method: "POST",
