...
const SKIP_EXACT = new Set([
  ...
  "в ", "во ",
  // Экран «сайт не смог запуститься» двуязычен по замыслу, а не по недосмотру: он появляется
  // именно тогда, когда что-то не загрузилось, — и словарь языка может быть ровно тем, что не
  // загрузилось. Поэтому обе строки написаны рядом и переводу не подлежат.
  "Сайт не смог запуститься",
  "Скорее всего пропала связь. Проверь интернет и обнови страницу.",
  "Обновить · Yenilə",
  // Это не надпись, а реплика, которую получает Claude, чтобы начать разговор. На экране её
  // никто не видит, и переводить её некуда.
  "(Ученик зашёл. Поздоровайся и начни разговор.)",
]);
